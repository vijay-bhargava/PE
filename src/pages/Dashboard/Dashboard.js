...
import {
  Badge as Badgemui, Button, Checkbox, FormControl,
  FormControlLabel, InputAdornment, Menu, MenuItem,
  Select, TextField, CardContent, Typography, Box,
  Chip, Table, TableBody, TableCell, TableContainer,
  TableHead, TableRow, Grid, Stack, Tooltip as MuiTooltip
} from '@mui/material'
import PEModal from "../../components/PEModal";
import { HiOutlineFilter, HiOutlineSearch } from 'react-icons/hi'
import { LocalizationProvider, DatePicker } from "@mui/x-date-pickers";
import { AdapterDateFns } from '@mui/x-date-pickers/AdapterDateFns';
import { PieChart, Pie, Sector, BarChart, Bar, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer, Cell } from 'recharts';
import { useNavigate } from 'react-router-dom';
import { getFirstNCharacters } from "../../utils/common";

import { actionTypes, useStateValue } from "../../../src/store";
import { GetDashboardActivities } from '../../utils/activities';
...
import { Heading4, Text } from '../../global/components/Text';
  ...
  return (
    <div className="dashboard-container">
                          ...
                          {eventStatusData.map((item, index) => (
                            ...
                            >
                              ...
                              >
                                ...
                                ></Box>
                                <Typography
                                  className={`dashboard-pie-chart-label ${selectedIndex === index ? "selected" : ""
                              ...
                              >
                                <Typography
                                  className={`dashboard-pie-chart-value ${eventStatusData.length > 4 ? "small" : ""
                                    }`}
                                  style={{
                                    color:
      ...
      <PEModal
        ...
        size="lg"
        title={`All Activities (${getFilteredActivities().length})`}
        bodyStyle={{ padding: 0 }}
      >
        {/* Search and Filter Controls */}
        <Box sx={{ px: 2, py: 1.5, borderBottom: "1px solid #f0f0f0", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 2 }}>
          <TextField
            size="small"
            placeholder="Search activities..."
            ...
            sx={{ width: 280, "& .MuiOutlinedInput-root": { borderRadius: 1.5 } }}
          />
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            <Button variant="outlined" size="small" onClick={() => setShowModalFilters(!showModalFilters)} sx={{ textTransform: "none", fontSize: "0.75rem", borderRadius: 1.5, px: 2, py: 0.5 }}>
              🔽 Advanced Filters
            </Button>
            <Button variant="contained" size="small" onClick={applyFilters} sx={{ textTransform: "none", fontSize: "0.75rem", borderRadius: 1.5, px: 2, py: 0.5, backgroundColor: "#1565c0", "&:hover": { backgroundColor: "#0d47a1" } }}>
              Apply Filter
            </Button>
            <Button variant="outlined" size="small" onClick={clearActivityFilters} sx={{ textTransform: "none", fontSize: "0.75rem", borderRadius: 1.5, px: 2, py: 0.5, color: "#d32f2f", borderColor: "#d32f2f", "&:hover": { backgroundColor: "#ffebee", borderColor: "#c62828" } }}>
              Clear All Filter
            </Button>
          </Box>
        </Box>
