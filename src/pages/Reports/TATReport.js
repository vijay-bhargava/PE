...
const TATReport = () => {
    ...
    const LOCAL_STORAGE_KEY = 'TATReport_ColumnVisibility';
    useEffect(() => {
        ...
        renderCell: (params) => {
            return (
                <div
                    ...
                    onClick={() => {
                        navigate(`/configuration/manage-nfa/${params?.row.id}`); // Navigate to the desired page
                    }}
                    className='pointer'>
    ...
    const location = useLocation();
        ...
        const pullNFASummaryReport = async (
    pageNumber = 1,
    pageSize = 10,
    filterData = null,
    eventType = "NFA"   // 👈 add eventType parameter
) => {
    setLoading(true);
    ...
    try {
        ...
        const res = await apiClient.get(
            `api/NFAManage/TATReport?${queryParams}`,
            atoken
        );
