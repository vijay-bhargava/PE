...
const RFQSummaryReport = () => {
	...
	const LOCAL_STORAGE_KEY = 'RFQSummaryReportColumnVisibility';
					...
					if (item?.columnName === 'subject') {
						return (
							<div className="rfq-v2-cell" onClick={() => navigate(`/configuration/manage-rfq/${params?.row.id}`)}>
								<span className="rfq-v2-cell-subject" title={params.value || ''}>
	...
	const clear = async () => {
		...
		setPage(1);
		setRfqLoading(true);
		await pullRFQSummaryReport(1, pageSize);
		setRfqLoading(false);
	};
	...
	const handleColumnVisibilityReset = () => {
		const defaultVis = { id: false, eventCode: false };
