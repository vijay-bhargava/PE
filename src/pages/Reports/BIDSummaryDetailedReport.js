...
const BIDSummaryDetailedReport = () => {
			...
			return {
				...
				renderCell: (params) => {
					if (item?.columnName === 'subject') {
						return (
							<div className="rfq-v2-cell" onClick={() => navigate(`/configuration/manage-auction/${params?.row?.bidId || params?.row?.id}`)}>
					...
					return (
						<Box sx={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', cursor: 'pointer' }}
							onClick={() => navigate(`/configuration/manage-auction/${params?.row?.bidId || params?.row?.id}`)}>
	...
	const pullReportColumns = async () => {
		try {
			...
			if (res?.length > 0) { setTableColumnLabels(res); pullBIDSummaryDetailedReport(); }
	...
	const pullBIDSummaryDetailedReport = async (pageNumber = 1, pageSize = 10, filterData = null) => {
		setLoading(true);
		try {
			await updateToken();
			const queryParams = buildQueryParams({ CustomerId: customerid, PageNumber: pageNumber, PageSize: pageSize, ...(filterData || {}) });
			const res = await apiClient.get(`api/AuctionManage/BIDSummaryDetailedReport?${queryParams}`, atoken);
	...
	const handleFilterSubmit = async (filterValues) => {
		...
		try {
			...
			setPage(1);
			await pullBIDSummaryDetailedReport(1, pageSize, filterData);
			setRfqLoading(false);
	...
	const clear = async () => {
		...
		setPage(1);
		setRfqLoading(true);
		await pullBIDSummaryDetailedReport(1, pageSize);
		setRfqLoading(false);
	};
	...
	const handleExportClick = async () => {
		try {
			setLoading(true);
			const payload = { reportName: "BIDSummaryDetailedReport", customerId: customerid, area: "AuctionManage", timeZoneId: userDetail?.timeZone };
			...
			link.download = `BIDSummaryDetailedReport_${formatted}.xlsx`;
			...
			<div className="rfq-v2-page">
				<div className="rfq-v2-page-header">
					<div className="rfq-v2-breadcrumb">
						...
						<span>BID Summary Detailed Report</span>
					</div>
				</div>
						...
						<PETable
							...
							onPaginationModelChange={(model) => {
								...
								if (model.page !== (page - 1)) {
									...
									pullBIDSummaryDetailedReport(model.page + 1, model.pageSize, currentFilters);
								}
								if (model.pageSize !== pageSize) {
									setPageSize(model.pageSize);
									setPage(1);
									pullBIDSummaryDetailedReport(1, model.pageSize, currentFilters);
								}
							}}
							columnVisibilityModel={columnVisibilityModel}
