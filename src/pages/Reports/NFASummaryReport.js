...
const NFASummaryReport = () => {
	...
	const LOCAL_STORAGE_KEY = 'NFASummaryReportColumnVisibility';
			...
			return {
				...
				renderCell: (params) => {
					if (item?.columnName === 'subject') {
						return (
							<div className="rfq-v2-cell" onClick={() => navigate(`/configuration/manage-nfa/${params?.row?.id}`)}>
					...
					return (
						<Box sx={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', cursor: 'pointer' }}
							onClick={() => navigate(`/configuration/manage-nfa/${params?.row?.id}`)}>
	...
	const pullReportColumns = async () => {
		try {
			...
			if (res?.length > 0) { setTableColumnLabels(res); pullNFASummaryReport(); }
	...
	const pullNFASummaryReport = async (pageNumber = 1, pageSz = 10, filterData = null) => {
		setLoading(true);
		try {
			await updateToken();
			const queryParams = buildQueryParams({ CustomerId: customerid, PageNumber: pageNumber, PageSize: pageSz, ...(filterData || {}) });
			const res = await apiClient.get(`api/NFAManage/NFASummaryReport?${queryParams}`, atoken);
	...
	const handleFilterSubmit = async (filterValues) => {
		...
		try {
			...
			setPage(1);
			await pullNFASummaryReport(1, pageSize, filterData);
			setnfaLoading(false);
		} catch (error) { setnfaLoading(false); }
	};
	...
	const clear = async () => {
		...
		setPage(1);
		setnfaLoading(true);
		await pullNFASummaryReport(1, pageSize);
		setnfaLoading(false);
	};
	...
	const handleExportClick = async () => {
		try {
			setLoading(true);
			const payload = { reportName: "NFASummaryReport", customerId: customerid, area: "NFAManage", timeZoneId: userDetail?.timeZone };
						...
						advFilterPanel={(
							<form className="rfq-v2-filter-body"
								...
								<div className="rfq-v2-filter-footer">
									...
									<LoadingButton type="submit" loading={nfaLoading}
						...
						<PETable
							...
							onPaginationModelChange={(model) => {
								...
								if (model.page !== (page - 1)) {
									...
									pullNFASummaryReport(model.page + 1, model.pageSize, currentFilters);
								}
								if (model.pageSize !== pageSize) {
									setPageSize(model.pageSize);
									setPage(1);
									pullNFASummaryReport(1, model.pageSize, currentFilters);
								}
							}}
							columnVisibilityModel={columnVisibilityModel}
...
export default NFASummaryReport;
