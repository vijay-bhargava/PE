...
import { formatDateViaLocale, formatDateViaLocaleonlydatenottime, getBIDSummaryReport, getReportColumns, getRFQSummaryReport } from '../../utils/common/utility';
...
const NFASummaryReport = () => {
    ...
    const LOCAL_STORAGE_KEY = 'NFASummaryReport_ColumnVisibility';
    useEffect(() => {
    ...
    const clear = async () => {
        ...
        await pullNFASummaryReport(1, pageSize, null);
...
const pullNFASummaryReport = async (pageNumber = 1, pageSize = 10, filterData = null) => {
    setLoading(true);
    ...
    try {
        ...
        const res = await apiClient.get(
            `api/NFAManage/NFASummaryReport?${queryParams}`,
            atoken
        );
    ...
    const handleFilterSubmit = async (filterValues) => {
        ...
        try {
            ...
            // Call pullNFASummaryReport with filters, resetting to page 1
    ...
    >
      <div className="d-flex justify-content-between border-bottom align-items-center mb-3 mt-2">
        <div className="page-heading text-dark-blue textMedium">
          <BackButton title="NFA Summary Report" />
        </div>
      </div>
      {activeFiltersCount > 0 && (
