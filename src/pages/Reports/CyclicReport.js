...
import { formatDateViaLocale, formatDateViaLocaleonlydatenottime, getReportColumns, getCyclicSummaryReport } from '../../utils/common/utility';
...
//import FilterCyclicCell from '../Configuration/RequestForQuotation/FilterCyclicCell';
...
const CyclicSummaryReport = () => {
	...
	const columns = tableColumnLabels?.map(item => ({
		...
		// Replaced minWidth with width and increased base values
		width: item?.columnName == "subject" ? 220 : 150,
		// Removed flex: 1 to force native horizontal scrolling
		editable: false,
		...
		renderCell: (params) => {
			...
			return (
				<Box
					...
					onClick={() => {
						navigate(`/ configuration / manage - cyclic / ${params?.row.id} `);
					}}
				>
	...
	const pullCyclicSummaryReport = async (pageNumber = 1, pageSize = 10, filterData = null) => {
		...
		try {
			...
			const res = await apiClient.get(
				`api / CyclicManage / CyclicSummaryReport ? ${queryParams} `,
				atoken
			);
	...
	const handleExportClick = async () => {
		try {
			setLoading(true);
			const payload = {
				reportName: "CyclicSummaryReport",
				customerId: customerid,
				area: "CyclicManage",
			...
			const fileName = `CyclicSummaryReport_${formatted}.xlsx`;
				...
				>
					<div className="d-flex justify-content-between border-bottom align-items-center mb-3 mt-2">
						<div className="page-heading text-dark-blue textMedium">
							<BackButton title="Cyclic Report" />
						</div>
					</div>
					{activeFiltersCount > 0 && (
						...
						<div className="col-12 col-md-12 col-lg-12">
							<Box sx={{ width: '100%', height: 'calc(100vh - 150px)', mt: 2 }}>
								<DataGrid
									...
									// ✅ Set fixed height and scrolling behavior
									sx={{
										height: '100%',
										width: '100%',
