...
import { formatDateViaLocale, getReportColumns } from '../../utils/common/utility';
...
const BIDSavingSummaryReport = () => {
	...
	const LOCAL_STORAGE_KEY = 'BIDSavingSummaryReportColumnVisibility';
		...
		?.map(item => {
			...
			return {
				...
				valueFormatter: isDateCol
					? (params) => params.value ? formatDateViaLocale(params.value, userDetail) : ""
					: undefined,
				renderCell: (params) => {
	...
	const pullReportColumns = async () => {
		try {
			const data = { slug: 'BIDSavingSummaryReport', customerId: customerid };
			const res = await getReportColumns(data, atoken);
			if (res?.length > 0) {
				setTableColumnLabels(res);
				pullBIDSavingSummaryReport();
			} else {
				setLoading(false);
			}
		} catch (error) {
	...
	const pullBIDSavingSummaryReport = async (pageNumber = 1, pageSize = 10, filterData = null) => {
		setLoading(true);
		try {
			await updateToken();
			...
			const res = await apiClient.get(`api/AuctionManage/BIdSavingSummaryReport?${queryParams}`, atoken);
	...
	const handleFilterSubmit = async (filterValues) => {
		...
		try {
			...
			setPage(1);
			await pullBIDSavingSummaryReport(1, pageSize, filterData);
			setRfqLoading(false);
		} catch (error) {
			setRfqLoading(false);
		}
	...
	const clear = async () => {
		...
		setPage(1);
		setRfqLoading(true);
		await pullBIDSavingSummaryReport(1, pageSize);
		setRfqLoading(false);
	};
	...
	const handleColumnVisibilityReset = () => {
		const defaultVis = {};
		setColumnVisibilityModel(defaultVis);
		localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(defaultVis));
	};
	...
	const handleExportClick = async () => {
		try {
			setLoading(true);
			const payload = {
				reportName: "BIDSavingSummaryReport",
				customerId: customerid,
				area: "AuctionManage",
			...
			link.download = `BIDSavingSummaryReport_${formatted}.xlsx`;
	...
	useEffect(() => {
		if (atoken && customerid) { pullReportColumns(); }
	}, [atoken, customerid]);
			...
			<div className="rfq-v2-page">
				<div className="rfq-v2-page-header">
					<div className="rfq-v2-breadcrumb">
						...
						<span>Reports</span>
						<span className="rfq-v2-breadcrumb-sep">/</span>
						<span>BID Saving Summary Report</span>
					</div>
				</div>
								...
								<div className="rfq-v2-filter-fields">
									...
									<div>
										...
										<FormControl fullWidth>
											<Select displayEmpty id="Status" variant="outlined" size="small"
												value={formik.values.Status}
												onChange={(e) => formik.setFieldValue("Status", e.target.value)}>
												{["Open", "Draft", "Under Pre Approval", "Running", "Awarded", "Forwarded", "Cancel", "Close"].map((s) => (
									...
									<LocalizationProvider dateAdapter={AdapterDateFns}>
										<div>
											...
											<MobileDateTimePicker className="w-100 f14" value={formik.values.FromDate} onChange={(v) => formik.setFieldValue("FromDate", v)} slotProps={{ textField: { variant: "outlined", size: "small" } }} />
										</div>
										<div>
											<label className="rfq-v2-filter-label">To Date/Time</label>
											<MobileDateTimePicker className="w-100 f14" value={formik.values.ToDate} onChange={(v) => formik.setFieldValue("ToDate", v)} slotProps={{ textField: { variant: "outlined", size: "small" } }} />
										</div>
									</LocalizationProvider>
								</div>
								<div className="rfq-v2-filter-footer">
									<button type="button" className="rfq-v2-filter-btn-reset" onClick={clear}>Reset</button>
									<LoadingButton type="submit" loading={rfqLoading} className="rfq-v2-filter-btn-apply" disableElevation onClick={async (e) => { e.preventDefault(); formik.handleSubmit(); }}>Apply</LoadingButton>
								</div>
							</form>
						)}
						showExport onExport={handleExportClick}
						...
						<PETable
							...
							onPaginationModelChange={(model) => {
								...
								if (model.page !== (page - 1)) {
									...
									pullBIDSavingSummaryReport(model.page + 1, model.pageSize, currentFilters);
								}
								if (model.pageSize !== pageSize) {
									setPageSize(model.pageSize);
									setPage(1);
									pullBIDSavingSummaryReport(1, model.pageSize, currentFilters);
								}
							}}
							columnVisibilityModel={columnVisibilityModel}
...
export default BIDSavingSummaryReport;
