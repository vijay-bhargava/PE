...
const RFQSavingSummaryReport = () => {
	...
	const pullReportColumns = async () => {
		try {
			...
			if (res?.length > 0) { setTableColumnLabels(res); pullRFQSavingSummaryReport(); }
	...
	const pullRFQSavingSummaryReport = async (pageNumber = 1, pageSz = 10, filterData = null) => {
		setLoading(true);
		try {
			await updateToken();
			const queryParams = buildQueryParams({ CustomerId: customerid, PageNumber: pageNumber, PageSize: pageSz, ...(filterData || {}) });
			const res = await apiClient.get(`api/RFQManage/RFQSavingSummaryReport?${queryParams}`, atoken);
	...
	const handleFilterSubmit = async (filterValues) => {
		...
		try {
			...
			setPage(1);
			await pullRFQSavingSummaryReport(1, pageSize, filterData);
			setRfqLoading(false);
	...
	const clear = async () => {
		...
		setPage(1);
		setRfqLoading(true);
		await pullRFQSavingSummaryReport(1, pageSize);
		setRfqLoading(false);
	};
	...
	const handleColumnVisibilityReset = () => {
		const d = { id: false }; setColumnVisibilityModel(d);
	...
	const handleExportClick = async () => {
		try {
			setLoading(true);
			const payload = { reportName: "RFQSavingSummaryReport", customerId: customerid, area: "RFQManage", timeZoneId: userDetail?.timeZone };
			...
			link.download = `RFQSavingSummaryReport_${formatted}.xlsx`;
			...
			<div className="rfq-v2-page">
				<div className="rfq-v2-page-header">
					<div className="rfq-v2-breadcrumb">
						...
						<span>RFQ Saving Summary Report</span>
					</div>
				</div>
								...
								<div className="rfq-v2-filter-fields">
									...
									<div>
										...
										<FormControl fullWidth>
											<Select displayEmpty id="stage" variant="outlined" size="small" value={formik.values.stage}
						...
						<PETable
							...
							onPaginationModelChange={(model) => {
								...
								if (model.page !== (page - 1)) {
									...
									pullRFQSavingSummaryReport(model.page + 1, model.pageSize, currentFilters);
								}
								if (model.pageSize !== pageSize) {
									setPageSize(model.pageSize); setPage(1);
									pullRFQSavingSummaryReport(1, model.pageSize, currentFilters);
								}
							}}
							columnVisibilityModel={columnVisibilityModel}
