...
const RFQSummaryDetailedReport = () => {
    ...
    const columns = tableColumnLabels?.map(item => ({
        ...
        minWidth: (item?.columnName == "subject" || item?.columnName == "itemName" || item?.columnName == "supplierCompanyName" ) ? 180 : 100,
        flex: 1,
        editable: false,
        ...
        valueFormatter: (item?.columnTitle === "Start Date" || item?.columnTitle === "End Date" || item?.columnTitle === "Configure Date")
            ...
            : undefined,
        renderCell: (params) => {
            const isSubject = (item?.columnName == "subject" || item?.columnName == "itemName" || item?.columnName == "supplierCompanyName" || item?.columnName == "stage" ); // 👈 key check
...
const pullRFQSummaryReport = async (pageNumber = 1, pageSize = 10, filterData = null) => {
    ...
    try {
        ...
        const res = await apiClient.get(
            `api/RFQManage/RFQSummaryDetailedReport?${queryParams}`,
            atoken
        );
  ...
  setTotalCount(totalRecords);
   ...
   const handleExportClick = async () => {
    try {
        ...
        const fileName = `RFQSummaryDetailedReport_${formatted}.xlsx`;
    ...
    >
      <div className="d-flex justify-content-between border-bottom align-items-center mb-3 mt-2">
        <div className="page-heading text-dark-blue textMedium">
          <BackButton title="RFQ Summary Detailed Report" />
        </div>
      </div>
      {activeFiltersCount > 0 && (
