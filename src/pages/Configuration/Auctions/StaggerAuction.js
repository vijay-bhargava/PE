...
const StaggerAuction = ({ actions }) => {
    ...
    //console.log("timeRemainingForGrp::", timeRemainingForGrp)
    ...
    // Get the current group number based on the current index
    const currentGroupNo = groupNos[currentTableIndex];
    // Filter the items based on the current group number
    //const filteredItems = bidParamaterData?.filter(item => item.groupNo == currentGroupNo);
