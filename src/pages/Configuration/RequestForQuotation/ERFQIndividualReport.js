...
import {
	Tabs, Tab, Typography, Button, Collapse, Tooltip,
	Menu, MenuItem, IconButton, Box, Drawer, TextField,
	InputAdornment, AccordionDetails, Accordion, AccordionSummary
} from '@mui/material';
...
const ERFQIndividualReport = ({ accessLevel }) => {
		...
		<div>
			...
			<div className=''>
					...
					<Collapse in={expanded}>
						<Card.Body>
							<div className='row mx-1'>
								<div class="col-md-3 ms-0 ps-0 d-flex"><span class="f14 fw500 me-1">Subject: </span><span class="f14 fw400 text-truncate" title={rfqheader?.[0]?.subject ?? ""}>{rfqheader?.[0]?.subject ?? ""}</span></div>
								<div class="col-md-3 ms-0 ps-0 d-flex"><span class="f14 fw500 me-1">Currency: </span><span class="f14 fw400 text-truncate" title={rfqheader?.[0]?.baseCurrency}>{rfqheader?.[0]?.baseCurrency}</span></div>

								<div class="col-md-3 ms-0 ps-0 d-flex"><span class="f14 fw500 me-1">Start Date: </span><span class="f14 fw400 text-truncate" >{rfqheader?.[0]?.startDate ? formatDateViaLocale(
									rfqheader?.[0]?.startDate,
									userDetail
								) : ""}</span></div>
								<div class="col-md-3 ms-0 ps-0 d-flex"><span class="f14 fw500 me-1">End Date: </span><span class="f14 fw400 text-truncate" >{rfqheader?.[0]?.endDate ? formatDateViaLocale(
									rfqheader?.[0]?.endDate,
									userDetail
								) : ""}</span></div>
								<div class="col-md-3 ms-0 ps-0 d-flex"><span class="f14 fw500 me-1">Supplier:</span><span class="f14 fw400 text-truncate" title={supplierdetails?.[0]?.tradeName}>{supplierdetails?.[0]?.tradeName}</span></div>
								<div class="col-md-3 ms-0 ps-0 d-flex"><span class="f14 fw500 me-1">Supplier Currency: </span><span class="f14 fw400 text-truncate" title={supplierdetails?.[0]?.acceptedCurrency}>{supplierdetails?.[0]?.acceptedCurrency}</span></div>
								<div class="col-md-3 ms-0 ps-0 d-flex"><span class="f14 fw500 me-1">User:</span><span class="f14 fw400 text-truncate" title={supplierdetails?.[0]?.email}>{supplierdetails?.[0]?.email}</span></div>
								<div class="col-md-3 ms-0 ps-0 d-flex"><span class="f14 fw500 me-1">Status:</span><span class="f14 fw400 text-truncate" title={supplierdetails?.[0]?.status}>{supplierdetails?.[0]?.status}</span></div>
								<div class="col-md-3 ms-0 ps-0 d-flex"><span class="f14 fw500 me-1">Supplier Remark: </span><span class="f14 fw400 text-truncate" title={supplierdetails?.[0]?.remarks}> {supplierdetails?.remarks?.length > 20 ?
									<Tooltip title={supplierdetails?.[0]?.remarks}>
										{supplierdetails?.[0]?.remarks ? `${supplierdetails[0].remarks.slice(0, 20)}...` : '-'}
									</Tooltip> : supplierdetails?.[0]?.remarks ?? "N/A"}</span></div>
								<div className="col-md-12 ms-0 ps-0 me-0 pe-0">
									<div className="">
										<Accordion defaultExpanded elevation={0} disableGutters>
											...
											>
												<span className="f14 fw500"> Description </span>
											</AccordionSummary>
											<AccordionDetails className="f14 ps-1">
								...
								<div className="col-md-12 mt-2 ms-0 ps-0 me-0 pe-0">
									<div className="">
										<Accordion defaultExpanded elevation={0} disableGutters>
											<AccordionSummary
												expandIcon={<HiChevronDown />}
											...
											>
												<span className="f14 fw500"> Terms & Condition: </span>
											</AccordionSummary>
											<AccordionDetails className="f14 ps-1">
				...
				<div className="container-fluid mt-2">
					<div className="row justify-content-between align-items-baseline mt-0">
							...
							<div className={"col-12 col-md-12 col-lg-12 mb-2  "}>
								<Box sx={{ width: "100%", display: "flex", alignItems: "center" }}>
									...
									>
										...
										{<Tab value={3} className="text-capitalize f12 fw500" label="Supplier Attachment" />}
									</Tabs>
								</Box>
							</div>
						</div>
					...
					{value === 3 && <SupplierAttachmentCell
						eventid={eventid}
						eventtype={`RFQ`}
						satoken={atoken}
						vendorid={supplierid}
...
export default ERFQIndividualReport;
