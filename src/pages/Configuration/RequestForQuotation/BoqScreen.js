...
import { HiOutlineTrash, HiPlusSm, HiChevronDown, HiChevronRight } from 'react-icons/hi';
            ...
            <div className="d-flex justify-content-between align-items-center mb-2">
                ...
                {!readOnly && (
                    <div className="d-flex align-items-center gap-2">
                        {/* <LoadingButton variant="outlined" size="small" startIcon={<HiPlusSm />} onClick={addRow}>
                            Add Row
                        </LoadingButton> */}

                        {/* <LoadingButton variant="outlined" size="small" startIcon={<HiPlusSm />} onClick={addGroup}>
                            Add Group
                        </LoadingButton> */}
                        <Button
                            variant="outlined"
                ...
                <table className="w-100 f13" style={{ borderCollapse: 'collapse' }}>
                    <thead>
                        <tr style={{ background: '#f5f5f5', borderBottom: '2px solid #dee2e6' }}>
                            ...
                            <th className="p-2 text-start">Item Description</th>
                            <th className="p-2 text-center" style={{ width: 120 }}>Quantity</th>
                            <th className="p-2 text-center" style={{ width: 80 }}>UOM</th>
                            {/* <th className="p-2 text-end" style={{ width: 120 }}>Target Price</th> */}
                            {/* <th className="p-2 text-center" style={{ width: 80 }}>Tax %</th> */}
                            {/* <th className="p-2 text-end" style={{ width: 140 }}>Line Total</th> */}
                            {/* {!readOnly && <th className="p-2 text-center" style={{ width: 60 }}></th>} */}
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        {flatRows.length === 0 && !loading ? (
                            <tr>
                                <td colSpan={stage === "Open" ? 5 : 4} className="p-4 text-center text-muted">
                                    No BOQ data available. {!readOnly && "Upload an Excel file or add items manually."}
                                </td>
                            </tr>
                        ) : (
                            flatRows.map((row) => {
                                ...
                                return (
                            <tr key={row.id} style={{ borderBottom: '1px solid #dee2e6', background: '#fff' }}>
                                {stage === "Open" && (
                                    <td className="p-2 text-center">
                                        {!row.isGroup && (
                                            <Tooltip 
                                                title={isDisabled ? "Item already linked to an event" : ""}
                                                placement="top"
                                            >
                                                <span>
                                                    <input 
                                                        type="checkbox" 
                                                        checked={isSelected}
                                                        onChange={() => handleCheckboxChange(itemId, row)}
                                ...
                                <td className="p-2">
                                    <div style={{ paddingLeft: `${(row.level - 1) * 18}px`, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        {row.isGroup ? (
                                            <IconButton size="small" onClick={() => toggleExpand(row.id)} className="p-0 me-1">
                                                {row.expanded ? <HiChevronDown className="f14 text-secondary" /> : <HiChevronRight className="f14 text-secondary" />}
                                            </IconButton>
                                        ) : <span style={{ width: 22, display: 'inline-block' }} />}
                                        {row.isNewItem ? (
                                            <TextField
                                                size="small"
                                                placeholder={row.isGroup ? "Enter group name" : "Enter item description"}
                                            ...
                                            <span className={row.isGroup ? 'fw-semibold' : ''}>{row.description}</span>
                                        )}
                                    </div>
                                </td>
                                ...
                                {/* <td className="p-2 text-end">{!row.isGroup && <strong>{formatCurrency(row.targetPrice)}</strong>}</td> */}
                                {/* <td className="p-2 text-center">{!row.isGroup && `${row.taxPercent}%`}</td> */}
                                {/* <td className="p-2 text-end">{!row.isGroup && <strong>{formatCurrency(row.lineTotal)}</strong>}</td> */}
                    ...
                    {/* <tfoot>
                        <tr style={{ borderTop: '2px solid #dee2e6', background: '#f5f5f5' }}>
                            <td colSpan={!readOnly ? 5 : 4} className="p-2 text-end fw-semibold">Subtotal:</td>
                            <td className="p-2 text-end fw-semibold">₹ {subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                        </tr>
                    </tfoot> */}
                </table>
            </div>
