...
const LoadingFactor = ({ isModal, rfqId, version, vendorId, initialFactors = [], baseCurrency, onSubmitSuccess }) => {
	...
	const handleSubmit = async () => {
		...
		const payload = updatedFactors.map(({ _key, ...f }) => ({ ...f, id: 0 }));
		const params = buildQueryParams({ VendorId: vendorId, Version: version });

		setLoading(true);
		try {
			const res = await new ApiClient(customersuffix).postres(
				`/api/RFQManage/${rfqId}/RFQLoadingFactor?${params}`,
				payload,
				atoken
			);
			if (res) {
				toast.success(
					editIndex !== null ? 'Loading factor updated successfully' : 'Loading factor added successfully',
					{ toastId: 'lf-save' }
				);
	...
	const columns = [
		...
		},
		{
			...
			flex: 1,
			minWidth: 110,
			renderCell: ({ row }) => row.factorType === 'A' ? row.loadingAmount : `${row.factorPerc}%`,
		},
		{
			field: 'action',
			headerName: 'Actions',
