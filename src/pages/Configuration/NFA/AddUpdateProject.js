...
const [purchGrpId, setpurchGrpId] = useState(0);
   ...
   const initialValues = {
        ...
        project: editRecordData?.project ? String(editRecordData.project) : "",
        purchGrpId: null,
        purchOrgId: null,
  ...
  try {
    ...
    if (res) {
      ...
      toast.success(
        ...
        { toastId: editRecordData?.id > 0 ? "projectUpdate" : "projectAdd" }
      );
    }
  } catch (error) {
...
const handleEdit = useCallback((data) => {
  ...
  if (OrgGroupId && purchaseGroupAllList.length > 0) {
    ...
    const prefilledDocument = () => {
        ...
        setproject(editRecordData?.project);
