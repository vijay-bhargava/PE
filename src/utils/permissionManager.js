	...
	parsePermissions(userAccess) {
		...
		userAccess.forEach(access => {
			try {
				...
				// claimValue may arrive as a JSON string OR already as a parsed object
				let claimValue = access.claimValue;
				if (typeof claimValue === 'string') {
					claimValue = JSON.parse(claimValue);
				}
				...
				// skip malformed entries rather than crashing the whole parser
				console.warn("[PermissionManager] skipping malformed claimValue:", access?.claimType, e.message);
			}
		});
